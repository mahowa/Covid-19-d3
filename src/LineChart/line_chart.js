...
export const initLineChart = (containerId, options) => {
  ...
  if (options.area) {
    // Add area fill
    svg.append("path").attr('class', `${options.lineMap[options.area.property]}-area area`);
    // add lines for area
    svg.append("line").attr('class', `${options.lineMap[options.area.property]}-area-line area-line left`);
    svg.append("line").attr('class', `${options.lineMap[options.area.property]}-area-line area-line bottom`);
...
export const createLineChart = async (options) => {
  ...
  const u = svg.select(".additional-axis").selectAll("line").data(yScale.ticks(numYTicks));
  ...
  // Remove old axis
  u.exit().transition().duration(animationDuration).style("opacity", 0).remove();
