...
const getAllData = async (list, all = false) => {
  ...
  if (list.includes("states") || all) {
    ...
    const raw = await d3.csv("https://raw.githubusercontent.com/nytimes/covid-19-data/master/us-states.csv"); // TODO compile list by state
    ...
    Object.entries(statesData).forEach(([name, data]) => {
      ...
      let totalCases = Number(((_data$ = data[0]) === null || _data$ === void 0 ? void 0 : _data$.cases) || 0);
      let totalDeaths = Number(((_data$2 = data[0]) === null || _data$2 === void 0 ? void 0 : _data$2.deaths) || 0);
  ...
  if (list.includes("colleges") || all) {
    ...
    data.colleges = await d3.csv("https://raw.githubusercontent.com/nytimes/covid-19-data/master/colleges/colleges.csv");
  }
  ...
  if (list.includes("mask_use") || all) {
    ...
    data.mask_use = await d3.csv("https://raw.githubusercontent.com/nytimes/covid-19-data/master/mask-use/mask-use-by-county.csv");
  }
  ...
  if (list.includes("excess_deaths") || all) {
    ...
    data.excess_deaths = await d3.csv("https://raw.githubusercontent.com/nytimes/covid-19-data/master/excess-deaths/deaths.csv");
  }
...
var _default = async () => getAllData([], true);

exports.default = _default;
},{"d3":"node_modules/d3/index.js"}],"src/LineChart/line_chart.js":[function(require,module,exports) {
"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.createLineChart = exports.initLineChart = void 0;
...
const initLineChart = (containerId, options) => {
  // Set for params on redrawing
  _containerId = containerId;
  _options = options; // Remove old svg if found

  d3.select(`#${containerId} svg`).remove();
  const svg = d3.select(`#${containerId}`).append("svg").attr("width", width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom).append("g").attr("transform", `translate(${margin.left}, ${margin.top})`).attr("id", 'main'); // Initialize X and Y axis

  const x = d3.scaleTime().range([0, width]);
  svg.append("g").attr("transform", "translate(0," + height + ")").attr("class", "xAxis").attr("transform", `translate(0,${height})`);
  ...
  options.lines.forEach((line, i) => {
    svg.append("path").attr("class", `${options.lineMap[line.property]} data-line`);
  });
  svg.append("g").attr("class", "additional-axis");

  if (options.area) {
    // Add area fill
    svg.append("path").attr("class", `${options.lineMap[options.area.property]}-area area`); // add lines for area

    svg.append("line").attr("class", `${options.lineMap[options.area.property]}-area-line area-line left`);
    svg.append("line").attr("class", `${options.lineMap[options.area.property]}-area-line area-line bottom`); // Hacktoberfest Issue: make gradient customizable (Hint: you can add the vars to the options.area variable)
    // add gradient to svg

    svg.append("linearGradient").attr("id", "temperature-gradient").attr("gradientUnits", "userSpaceOnUse");
  }
...
const createLineChart = async options => {
  var _filteredData$, _filteredData;

  const numYTicks = 6;
  const animationDuration = 1000;
  const svg = _svg;
  const x = _x;
  const y = _y; // Set for params on redrawing

  _options = options; // Use same name for lines and update data for params to trigger chart update
  ...
  const numPoints = filteredData.length; // Find start date and end date for X

  const firstDay = (_filteredData$ = filteredData[0]) === null || _filteredData$ === void 0 ? void 0 : _filteredData$.date;
  const lastDay = (_filteredData = filteredData[numPoints - 1]) === null || _filteredData === void 0 ? void 0 : _filteredData.date;
  const numMonths = Math.abs(firstDay.getMonth() - lastDay.getMonth()) + 1; // Find the max values for Y

  const maxCases = d3.max(filteredData, d => Number(d[options.lineMap[options.yProperty]]));
  const maxY = Math.round(maxCases + maxCases * 0.1); // Create our scales

  const xScale = x.domain([firstDay, lastDay]);
  const yScale = y.domain([0, maxY]); // Generate the lines

  const lines = options.lines.map(l => d3.line().x(d => xScale(new Date(d.date))).y(d => yScale(d[options.lineMap[l.property]]))); // Hacktoberfest Issue: and ability from options param to add custom class (hint use current class as a default)
  // Set up axis ticks
  // X Axis

  svg.selectAll(".xAxis").transition().duration(animationDuration).call(d3.axisBottom(xScale).tickFormat(t => d3.timeFormat("%B")(t)).tickSize(0).ticks(numMonths) // todo figure out why this isnt working the way we want
  // http://www.d3noob.org/2016/08/changing-number-of-ticks-on-axis-in.html
  ).selectAll("text").attr("stroke", "none").attr("y", 10).selectAll("text").attr("stroke", "none").attr("y", 10); // Y Axis

  const yAxis = svg.selectAll(".yAxis").transition().duration(animationDuration).call(d3.axisLeft(yScale).tickSize(0).ticks(numYTicks, "s"));
  yAxis.select("path").attr("class", "axis");
  yAxis.selectAll("text").attr("x", -10).attr("y", -4); // Add closing boxes around chart
  // Hacktoberfest Issue: add a custom class using the options variable (Hint look for other examples)

  svg.select(".sideXAxis").attr("x1", xScale(firstDay)).attr("y1", yScale(maxY)).attr("x2", xScale(lastDay)); // Top Horizontal Line

  svg.select(".sideYAxis").attr("y1", yScale(0)).attr("x1", xScale(lastDay)).attr("y2", yScale(maxY)).attr("x2", xScale(lastDay)); // Right Vertical Line
  // Get all additional axis lines and set with data

  var u = svg.select(".additional-axis").selectAll("line").data(yScale.ticks(numYTicks)); // Add new axis

  u.enter().append("line").attr("class", "axis").merge(u).transition().duration(animationDuration).attr("y1", (d, i) => yScale(d)).attr("x1", xScale(firstDay)).attr("x2", xScale(lastDay)).attr("y2", (d, i) => yScale(d)); // Remove old axis

  u.exit().transition().duration(animationDuration).style("opacity", 0).remove(); // Add chart data lines

  options.lines.forEach((line, i) => {
    svg.select(`.${options.lineMap[line.property]}`).datum(filteredData).transition().duration(animationDuration).attr("d", lines[i]);
  });

  if (options.area) {
    console.log('in area'); // Add area fill

    svg.selectAll(`.${options.lineMap[options.area.property]}-area`).datum(filteredData).attr("d", d3.area().x(d => xScale(new Date(d.date))).y0(yScale(0)).y1(d => yScale(d[options.lineMap[options.area.property]]))); // add lines for area

    svg.select(`.${options.lineMap[options.area.property]}-area-line.bottom`).attr("x1", xScale(firstDay)).attr("y1", yScale(0)).attr("x2", xScale(lastDay)).attr("y2", yScale(0));
    svg.select(`.${options.lineMap[options.area.property]}-area-line.left`).attr("x1", xScale(lastDay)).attr("y1", yScale(0)).attr("x2", xScale(lastDay)).attr("y2", yScale(options.accum ? maxCases : filteredData[numPoints - 1][options.lineMap[options.area.property]])); // Hacktoberfest Issue: make gradient customizable (Hint: you can add the vars to the options.area variable)
    // add gradient to svg

    svg.select(`#temperature-gradient`).attr("x1", 0).attr("y1", yScale(0)).attr("x2", 0).attr("y2", yScale(maxCases)).selectAll("stop").data([{
      offset: "0%",
      color: "transparent"
    }, {
      offset: "50%",
...
},{"d3":"node_modules/d3/index.js"}],"src/d3helpers.js":[function(require,module,exports) {
"use strict";
...
},{"d3":"node_modules/d3/index.js"}],"src/filters.js":[function(require,module,exports) {
"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.createSelectBox = exports.createCheckBox = void 0;
 ...
 */
const createCheckBox = (containerId, property, text, onChange, labelClass = "cv19label", inputClass = "cv19input") => {
  const id = `${containerId}_${property}_checkbox`;
  d3.select(`#${containerId}`).append("label").attr("class", labelClass).text(text).append("input").attr("type", "checkbox").attr("id", id).property("checked", false).attr("class", inputClass).on("change", () => {
...
const createSelectBox = (containerId, property, options, onChange, selectClass = "cv19select", optionClass = "cv19option") => {
  const id = `${property}_select`;
  const select = d3.select(`#${containerId}`).append("select").attr("class", selectClass).attr("id", id).on("change", () => {
    const val = d3.select(`#${id}`).property("value");
    onChange({
      [property]: val
    });
  });
  select.selectAll("option").data(options).enter().append("option").attr("class", optionClass).text(d => d);
};

exports.createSelectBox = createSelectBox;
},{"d3":"node_modules/d3/index.js"}],"src/LineChart/index.js":[function(require,module,exports) {
"use strict";
...
var _d3helpers = require("../d3helpers");
...
var lineChartConfig = {
  state: "Alabama",
  // HacktoberFest Issue: would be cool if we got the users location and set this to default
  accum: false
};
...
const build = data => {
  ...
  (0, _d3helpers.addContainer)(chartId);
  (0, _filters.createSelectBox)(chartId, "state", Object.keys(data.states).sort(), updateConfig);
  (0, _filters.createCheckBox)(chartId, "accum", "Show Cumulative", updateConfig);
  (0, _line_chart.initLineChart)(chartId, lineChartOptions); // Hacktoberfest issue: create a time selection control
...
var _default = build;
exports.default = _default;
},{"./line_chart":"src/LineChart/line_chart.js","../d3helpers":"src/d3helpers.js","../filters":"src/filters.js"}],"src/index.js":[function(require,module,exports) {
"use strict";
...
const main = async () => {
  ...
  // Get all data
  const data = await (0, _data.default)();
  console.log("Initialized data sets", Object.keys(data));
  (0, _LineChart.default)(data);
};

main();
},{"./data":"src/data.js","./LineChart":"src/LineChart/index.js"}],"node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
